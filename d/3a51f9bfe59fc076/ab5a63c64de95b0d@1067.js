...
import define7 from "./cc0400a7ad6d96d0@3027.js";
...
md`### Changes
- 2025-05-17 [Avoid cloning the remote](https://bsky.app/profile/larkworthy.bsky.social/post/3lpetshkbes2i) buy using a blank repository (rolled back as this does not work when overwriting existing files)
...
Inputs.bind(
  ...
  urlQueryFieldView("export_state", {
    defaultValue: JSON.stringify(
      {
        title: source_notebook,
        hash: `#view=${encodeURI(
          `R100(S70(${source_notebook}),S30(@tomlarkworthy/module-selection))`
        )}`
      },
      null,
      2
...
async function _frame_define(load_source,frame_notebook)
{
  ...
  return (
    await import(
      `https://api.observablehq.com/${frame_notebook}.js?v=4&resolutions=0000000000000000`
    )
  ).default;
}
...
async function _source_define(load_source,source_notebook)
{
  ...
  return (
    await import(
      `https://api.observablehq.com/${source_notebook}.js?v=4&resolutions=0000000000000000`
    )
  ).default;
}
...
function _embedded_source(embedded_frame,embedded_runtime,source_define){return(
embedded_frame && embedded_runtime.module(source_define)
)}
...
function _exported(exportToHTML,embedded_runtime,frame_notebook,embedded_frame,source_notebook,embedded_source,export_state,$0)
{
  debugger;
  return exportToHTML({
    runtime: embedded_runtime,
    mains: new Map([
      [frame_notebook, embedded_frame],
      [source_notebook, embedded_source]
    ]),
    options: {
      ...export_state,
      debug: false,
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("embedded_source")).define("embedded_source", ["embedded_frame","embedded_runtime","source_define"], _embedded_source);
  ...
  main.variable(observer("exported")).define("exported", ["exportToHTML","embedded_runtime","frame_notebook","embedded_frame","source_notebook","embedded_source","export_state","mutable output"], _exported);
  ...
  main.variable(observer()).define(["exporter"], _53);

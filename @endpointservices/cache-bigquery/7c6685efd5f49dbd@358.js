...
import define5 from "./698257e86fae4586@346.js";
import define6 from "./ef672b935bd480fc@619.js";
import define7 from "./293899bef371e135@216.js";
...
`
  ...
  main.variable(observer("encrypted_service_account_credentials")).define("encrypted_service_account_credentials", function(){return(
{
  name: "AES-GCM",
  salt: "x4gEPgJOp9brBg==",
  iv: "Hwoo2XyMFSJT5FLq",
...
md`## Import a UI to decrypt the service account`
  ...
  main.variable(observer("viewof service_account_credentials")).define("viewof service_account_credentials", ["decrypt","encrypted_service_account_credentials"], function(decrypt,encrypted_service_account_credentials){return(
decrypt(
...
md`Check it works by reading out its result. This will only resolve it you enter a valid password.`
  ...
  main.variable(observer()).define(["service_account_credentials"], function(service_account_credentials){return(
...
md`Once it works in that notebook we can bring it over to our target notebook`
  ...
  main.variable(observer("gapi_config")).define("gapi_config", ["service_account_credentials"], function(service_account_credentials){return(
{
  ...
  main.variable(observer("bqClient")).define("bqClient", ["createGapi","gapi_config"], async function(createGapi,gapi_config){return(
...
`
  ...
  main.variable(observer("bqResponse")).define("bqResponse", ["bqClient"], function(bqClient){return(
...
md`## Tidy results`
  ...
  main.variable(observer("data")).define("data", ["bqResponse"], function(bqResponse){return(
...
md`Now we can login using a service account identity. This is using a service account like a regular user on the internet. So in order to give them special privileges in Firebase Security Rules, we need to discover their uid.`
  ...
  main.variable(observer("service_account_user")).define("service_account_user", ["getAccessTokenFromServiceAccount","service_account_credentials","signinWithAccessToken","firebase"], async function(getAccessTokenFromServiceAccount,service_account_credentials,signinWithAccessToken,firebase)
{
...
md`Print their uid for use with rules`
  ...
  main.variable(observer()).define(["service_account_user"], function(service_account_user){return(
...
`
  ...
  main.variable(observer()).define(["service_account_user","data","firebase"], function(service_account_user,data,firebase)
{
...
`
  ...
  main.variable(observer("uploadToBucket")).define("uploadToBucket", ["getAccessTokenFromServiceAccount","service_account_credentials","data"], async function(getAccessTokenFromServiceAccount,service_account_credentials,data)
{
...
`
  ...
  main.variable(observer("cached_data_ref")).define("cached_data_ref", ["firebase"], function(firebase){return(
firebase
  .storage()
  .ref()
  .child("/examples/cache-bigquery/data.json")
)});
  main.variable(observer("cached_data")).define("cached_data", ["cached_data_ref"], async function(cached_data_ref){return(
JSON.parse(
...
md`Note we can display this data even without authenticating with Firebase!!! You could of course put the reading of Firebase data in a different notebook too`
  ...
  main.variable(observer()).define(["Table","cached_data"], function(Table,cached_data){return(
Table(cached_data)
)});

...
`
  ...
  main.variable(observer("Router")).define("Router", ["require"], function(require){return(
require('express-router-browserify@3.0.1')
)});
  main.variable(observer()).define(["md"], function(md){return(
md`### Write your app like a normal [Express](https://expressjs.com/) app`
)});
  main.variable(observer("app")).define("app", ["Router"], function(Router)
{
  const app = Router();
...
`
  ...
  main.variable(observer("handleWithExpress")).define("handleWithExpress", ["cookie_signature","cookie"], function(cookie_signature,cookie){return(
...
`
  ...
  main.variable(observer("test_router_endpoint")).define("test_router_endpoint", ["deploy","handleWithExpress","app"], function(deploy,handleWithExpress,app)
{
  ...
  main.variable(observer("cookie_signature")).define("cookie_signature", ["require","FileAttachment"], async function(require,FileAttachment){return(
require(await FileAttachment(
  "cookie-signature-1.1.0.js"
).url())
)});
  main.variable(observer("cookie")).define("cookie", function()
{
  /**
   * Module variables.
   * @private
...
`
  ...
  main.variable(observer("viewof ci")).define("viewof ci", ["viewof suite"], function($0){return(
$0
)});
  main.variable(observer("ci")).define("ci", ["Generators", "viewof ci"], (G, _) => G.input(_));
  main.variable(observer()).define(["ci","test_router_endpoint","expect"], function(ci,test_router_endpoint,expect){return(
...
ci.test("/setResponseHeader returns response with response header set", async () => {
  ...
  main.variable(observer()).define(["ci","expect","test_router_endpoint"], function(ci,expect,test_router_endpoint){return(

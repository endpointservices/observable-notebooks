...
(
  ...
  invalidation.then(
    configDoc.onSnapshot((snap) => {
      ...
      ui.value.livecode.livemodepublic =
        val?.flags?.livemodepublic === undefined
          ? false
          : val.flags.livemodepublic;
    ...
    configDoc.set(
      {
        ...
        flags: {
          ...
          livemodepublic: ui.livecode.singleton.livemodepublic.value
        }
      },
      { merge: true }
    );
...
({ livecode = true, livecodepublic = false } = {}) => {
  const ui = columnPane({
    content: view`<div class="e-col-title">Live Coding</div>
      <p class="e-explain"><i>Live coding tunnels production traffic to <b>your</b> browser so you can run and debug the latest serverside code reactively.</i></p>
      ${[
        "livemode",
        Inputs.toggle({
      ...
      ${[
        "livemodepublic",
        Inputs.toggle({
          label: "⚠️ enable public live coding",
          value: livecodepublic
        })
      ]}
      <p class="e-explain">tunnelled: ${["tunnelled", textNodeView(0)]}</p>
      <p class="e-explain"><i>⚠️ Bound secrets are transmitted</i></p>
  </div`
  });

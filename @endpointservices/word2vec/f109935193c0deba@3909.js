...
async function _on_prompt($0,system_prompt,$1,include_deps,prompt,$2,runAsk,settings)
{
...
function _runAsk(d3,modelConfig,OPENAI_API_KEY,api_endpoint,process){return(
async ({ settings, messages }) => {
  const prompt = [...messages];
  const prompt_tokens = prompt.map(
    (p) => (p.content || JSON.stringify(p.function_call)).length / 5
  );
  while (d3.sum(prompt_tokens) > settings.max_prompt_tokens) {
    prompt.splice(1, 1);
    prompt_tokens.splice(1, 1);
  }
  console.log("trimmed_prompt", prompt);

  const body = {
    //functions: functions,
    // function_call: { name: "upsert_cell" },
    messages: prompt,
    ...modelConfig(settings.model).settings
  };
  const payload = {
    method: "POST",
    headers: {
...
flowQueue({ name: "ask", timeout_ms: 180000 })
)}

function _58(runAsk,$0,prompt_messages){return(
runAsk({
  settings: $0.value,
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("on_prompt")).define("on_prompt", ["viewof settings","system_prompt","viewof contextRequest","include_deps","prompt","mutable context","runAsk","settings"], _on_prompt);
  ...
  main.variable(observer("runAsk")).define("runAsk", ["d3","modelConfig","OPENAI_API_KEY","api_endpoint","process"], _runAsk);
  ...
  main.variable(observer()).define(["runAsk","viewof settings","prompt_messages"], _58);
  main.variable(observer()).define(["ask"], _59);

...
# A Causally Consistent Redis Backend
</div>
...
Note the real Firebase Realtime Database server has more semantics than we have implemented. Its LISTENs can query and filter, and values can contain JSON documents, so this is far from a feature complete server backend. However, under the literate programming paradigm, I prefer each notebook to concentrate on a single topic. This notebook is about causal consistency. We will add query semantics in dedicated notebook later. They can all be found in the [Firebase collection](https://observablehq.com/collection/@tomlarkworthy/firebase).

I hope you have enjoyed this writeup! Please ❤️ at the top and spread the word!`
)}
...
export default function define(runtime, observer) {
  ...
  main.variable(observer()).define(["footer"], _132);
  return main;
}
